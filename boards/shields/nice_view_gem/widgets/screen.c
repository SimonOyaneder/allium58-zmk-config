...
// Arte: luna con Nika, 136x68 en orientación de framebuffer (68x136 físico).
// Se dibuja ENCIMA de los canvas de estado dejando dos franjas visibles:
// 16px arriba (icono de conexión + % de batería en una fila) y 8px abajo
// (solo los puntos del selector de perfil BT).
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    // Canvas de estado superior (hijo 0): extremo derecho del fb = arriba físico
    lv_obj_t *top = lv_canvas_create(widget->obj);
    lv_obj_align(top, LV_ALIGN_TOP_RIGHT, 0, 0);
    lv_canvas_set_buffer(top, widget->cbuf, CANVAS_SIZE, CANVAS_SIZE, CANVAS_COLOR_FORMAT);

    // Canvas de estado inferior (hijo 1): extremo izquierdo del fb = abajo físico
    ...
    // Arte encima (hijo 2): deja 8px abajo y 16px arriba de franjas visibles
    lv_obj_t *art = lv_img_create(widget->obj);
    lv_img_set_src(art, &moon_nika);
    lv_obj_align(art, LV_ALIGN_TOP_LEFT, 8, 0);
